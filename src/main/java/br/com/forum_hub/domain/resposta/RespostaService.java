...
public class RespostaService {
    ...
    public Resposta marcarComoSolucao(Long id, Usuario logado) {
        ...
        if(!usuarioTemPermissoes(logado, topico.getAutor())) {
            throw new AccessDeniedException("Voce não pode marcar essa resposta como solucao.");
        }

        if(topico.getStatus() == Status.RESOLVIDO)
            throw new RegraDeNegocioException("O tópico já foi solucionado! Você não pode " +
                    "marcar mais de uma resposta como solução.");
    ...
    private boolean usuarioTemPermissoes(Usuario logado, Usuario autor) {
        for(GrantedAuthority autoridade: logado.getAuthorities()) {
            var autoridadesAlcancaveis = this.roleHierarchy
                    .getReachableGrantedAuthorities(List.of(autoridade));
